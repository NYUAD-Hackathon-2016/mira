...
public class UsbController {
    ...
    public UsbController(Activity parentActivity,
                         IUsbConnectionHandler connectionHandler, int vid, int pid, MainActivity owner, TextView textView) {
        this.owner = owner;
    ...
    private class UsbRunnable implements Runnable {
        ...
        public void run() {//here the main USB functionality is implemented
            final UsbDeviceConnection conn = mUsbManager.openDevice(mDevice);
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    while (true) {
                        try {
                            conn.bulkTransfer(epIN, bytes, 1, 0);
                            eventTracker = bytes[0];
                            if (eventTracker == 0)
                                eventFired = false;
                            Thread.sleep(10);
                        } catch (Exception e) {
